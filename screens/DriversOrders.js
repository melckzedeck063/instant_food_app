...
import { getMyOrders, getSentOrders } from '../store/actions/order_actions';
  ...
  return (
    <View>
        <View style={{backgroundColor : '#0e2433'}} className={`bg-slate-900 h-full`}>
          ...
          {
            my_orders?.driver_orders?.data?.data.length >= 1?(
              <>
              <FlatList 
              data={my_orders.driver_orders.data.data}
                 renderItem={(itemData)  => {
                  return(
