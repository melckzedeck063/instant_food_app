...
import { getAllOrders } from '../store/actions/order_actions';
...
const AllOrders = () => {
    ...
    useEffect(() => {
      if(my_orders && my_orders.all_orders && reload  < 4){
  ...
  return (
    <View>
        <View style={{backgroundColor : '#0e2433'}} className={`bg-slate-900 h-full`}>
          ...
          <View style={{height  : responsiveHeight(89.5), width : responsiveWidth(98), alignSelf  :  'center'}} className="bg-whitee">
          {
            my_orders?.all_orders?.data?.data.length >= 1?(
              <>
              <FlatList 
              data={my_orders.all_orders.data.data}
                 renderItem={(itemData)  => {
                  return(
